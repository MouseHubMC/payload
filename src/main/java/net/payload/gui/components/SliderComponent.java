...
public class SliderComponent extends Component {
	...
	public void draw(DrawContext drawContext, float partialTicks) {
		...
		if (GuiManager.minifloat.getValue()) {
			...
			// Calculate the position for scaled text
			float scaleFactor = 0.5f;
			float yOffset = actualY + 13;

			// Translate to the drawing position, scale, and translate back
			matrixStack.translate(xPos, yOffset, 0);
			matrixStack.scale(scaleFactor, scaleFactor, 1.0f);
			matrixStack.translate(0, textWidth * 0.25f, 0); // Adjust vertical position after scaling

			// Draw the scaled text
			Render2D.drawString(drawContext, valueText, 0, 0, 0xFFFFFF);
